...
with graph.as_default():
    ...
    with tf.variable_scope("x1"):
        outputs1, states1 = tf.nn.rnn(D3rnn_cell, [x1], dtype=tf.float32)
        print(outputs1[0].name)

    with tf.variable_scope("x1", reuse=True):
        outputs2, states2 = tf.nn.rnn(D3rnn_cell, [x2], dtype=tf.float32)
        print(outputs2[0].name)
